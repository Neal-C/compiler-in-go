...
func TestErrorHandling(t *testing.T) {
	...
	for _, tt := range tableTests {
		...
		if errObj.Message != tt.expectedMessage {
			t.Errorf("wrong error message, exepcted = %q, got = %q", tt.expectedMessage, errObj.Message)
		}
	}
}
...
func TestFunctionObject(t *testing.T) {
	...
	if !ok {
		t.Fatalf("evaluated is not a function. got = %T (%v)", evaluated, evaluated)
	}

	if len(fn.Parameters) != 1 {
		t.Fatalf("fn has wrong parameters. Parameters = %+v", fn.Parameters)
	}
...
func TestStringLiteral(t *testing.T) {
	...
	if str.Value != "Hello world!" {
		t.Errorf("str.Value has the wrong value, got: %q", str.Value)
	}
...
func TestBuiltinFunctions(t *testing.T) {
	...
	for _, tt := range tableTests {
		...
		case string:
			...
			if !ok {
				t.Errorf("errorObj is not *object.Error, got = %T (%v)", evaluated, evaluated)
				continue
			}
...
func TestArrayLiteral(t *testing.T) {
	...
	if !ok {
		t.Fatalf("evaluated is not a *object.Array, got = %T (%v)", evaluated, evaluated)
	}
...
func TestHashLiterals(t *testing.T) {
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash has a wrong number of Paris. got %d , want %d", len(result.Pairs), len(expected))
	}
